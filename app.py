...
def crear_baraja_mus():
    ...
    # El mus usa: 1, 2, 3, 4, 5, 6, 7, 10, 11, 12
...
if st.session_state.mano:
    ...
    for i, carta in enumerate(st.session_state.mano):
        with cols[i]:
            # Convertimos el número al formato de dos dígitos (ej: 1 -> 01)
            num_str = str(carta['num']).zfill(2)
            # Construimos el nombre exacto: 01-oros.png
            ...
            if os.path.exists(ruta):
                ...
                st.error(f"No existe: {nombre_archivo}")
